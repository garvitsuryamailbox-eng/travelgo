...
export default function SignatureJourneysSection() {
  ...
  // Helper to extract numeric USD value from strings like "$12,500"
  const parseUSD = (priceStr: string) => {
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {signatureJourneysData.slice(0, 6).map((journey) => {
            const usdVal = parseUSD(journey.startingPrice);
            return (
              <div
                key={journey.id}
                  ...
                  <div className="pt-4 border-t border-[#c5a880]/15 flex items-center justify-between">
                    <div>
                      ...
                      <span className="font-serif text-xl font-semibold text-[#faf9f6]">
                        {formatPrice(usdVal)} <span className="text-xs font-sans text-[#eae6df]/50 font-normal">/ person</span>
                      </span>
                    </div>
